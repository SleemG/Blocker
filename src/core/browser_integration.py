...
class BrowserContentMonitor:
    ...
    def trigger_block_screen(self, url: str, reason: str):
        ...
        # This would communicate back to the main application
...
class RegistryBrowserMonitor:
    ...
    def _show_block_screen_direct(self, url: str, reason: str):
        ...
        try:
            ...
            from adult_content_blocker import BlockScreen, SettingsManager
            
            app = QApplication.instance()
            if not app:
                app = QApplication([])
            
            settings_manager = SettingsManager(self.database_path, self.user_email)
            
            block_screen = BlockScreen(
                message=settings_manager.get_block_message(),
                reason=f"Blocked: {reason}",
                countdown_seconds=settings_manager.get_countdown_seconds(),
                redirect_url=settings_manager.get_redirect_url()
            )
...
class ContentBlockerService:
    ...
    def _show_block_screen(self, url: str):
        ...
        try:
            ...
            settings = self._get_block_settings()
            ...
            # Create the block screen in the main thread
            def show_block_screen():
                try:
                    block_screen = MainWindow(
                        ...
                        reason=f"Blocked URL: {url}"
                    )
                    
                    # Connect to the block finished signal
                    def on_block_finished(redirect_url):
                        try:
                            import webbrowser
                    ...
                    block_screen.blockFinished.connect(on_block_finished)
                    block_screen.show()
                    
                except Exception as e:
                    print(f"Error creating block screen: {e}")
            
            # Schedule the block screen creation in the main thread
            QMetaObject.invokeMethod(app, show_block_screen, Qt.QueuedConnection)

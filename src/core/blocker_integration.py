...
class BlockerIntegration(QObject):
    ...
    def on_countdown_setting_changed(self, value: int):
        ...
        try:
            # Save the setting to database or configuration
            # This will be used by the BlockScreen
            self.save_setting('countdown_duration', value)
    ...
    def on_message_setting_changed(self):
        ...
        try:
            if hasattr(self.main_window, 'plainTextEdit'):
                ...
                # Save to settings and update any active block screens
                self.save_setting('block_message', message)
                # Update any active block screen if it exists
                if hasattr(self, 'block_screen') and self.block_screen is not None:
                    if hasattr(self.block_screen, 'msg_txtBrwsr'):
                        self.block_screen.msg_txtBrwsr.setText(message)
    ...
    def on_redirect_url_changed(self, url: str):
        ...
        try:
            self.save_setting('redirect_url', url)
    ...
    def show_block_screen(self, url: str, reason: str, detected_content: str):
        ...
        try:
            ...
            # Get settings for the block screen
            message = self.get_setting('block_message', self.get_default_block_message())
            countdown = self.get_setting('countdown_duration', 30)
            redirect_url = self.get_setting('redirect_url', 'https://www.google.com')
    ...
    def is_adult_blocking_enabled(self) -> bool:
        ...
        try:
            setting = self.main_window.db.get_setting(self.main_window.user_email, 'checkBox')
            return setting if setting is not None else True
        except Exception as e:
            print(f"Error checking adult blocking setting: {e}")
            return True

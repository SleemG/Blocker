...
class SignUpDialog(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Add temporary skip button for development
        self.skip_btn = QPushButton("Skip Verification (Development Only)", self)
        ...
        """)
        self.skip_btn.clicked.connect(self.skip_verification)
        # Add skip button to the layout
        self.signup_pg.layout().addWidget(self.skip_btn)
    ...
    def skip_verification(self):
        """Development only: Skip the verification process"""
        self.email = "dev@example.com"  # Set a default development email
        username = self.name_lineEdit.text().strip() or "Developer"
        # Show a warning that this is for development only
        QMessageBox.warning(self, "Development Mode", 
            "Skipping verification - This feature will be removed in production!")
        if self.db.add_user(self.email, username)[0]:
            self.code_verified.emit(self.email)
            self.close()  # Use close() for QMainWindow instead of accept()
